...
struct CustomBodyMainView: View {
    ...
    var body: some View {
        VStack{
            ...
            HStack {
                ForEach(dressTypes, id: \.self) { item in
                    HStack {
                        Button{
            ...
            ForEach(customData) { item in
                LazyVGrid(columns: Array(repeating: GridItem(), count: 2)) {
                    ForEach(item.myRows) { itemRow in
                        //cualquier cosa puede ser un NavigationLink porque hay un NavigationView en el padre
                        NavigationLink{
...
var dressTypes = ["Dress", "Pants", "Blazers", "Jackets"]
